...
import { CreateStore } from "@/action/createtienda-action";
...
export function ModalTienda() {
    ...
    const [logo, setLogo] = useState("/tienda.png")
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: (data: CreateStoreType) => CreateStore(data),
        onSuccess: (message) => {
    ...
    const { handleSubmit, register, reset, setValue, formState: { errors } } = useForm<CreateStoreType>()


    const onChangeProvince = (e: React.ChangeEvent<HTMLSelectElement>) => {
        const response = e.target.value;
        const provincia = datosProvince.provincia.find(n => n.nombre == response)
        setDistrito(provincia?.distrito)
    }

    const onChangeDistrito = (e: React.ChangeEvent<HTMLSelectElement>) => {
        const response = e.target.value;
        const distritos = distrito?.find(n => (n.nombre == response))
        setCorregimiento(distritos?.corregimientos)
    }

    if (isLoading) return <Spinner />
    return (
        <Modal placeholder="Add" action={handleSubmit((data) => mutate(data))} ispending={isPending}>

            <div className="flex flex-col-reverse items-center  justify-center">
                <h2>Logo de la Tienda</h2>
                <Image className="rounded-full" src={logo} width={80} height={80} alt="Logo" />
            ...
            <div>
                ...
                <select className="w-full p-2 mt-1 rounded-lg border" {...register("categoryId", { required: "La Categoria es necesaria para ubicar el negocio" })}>
            ...
            <div>
                ...
                <select className="w-full p-2 mt-1 rounded-lg border" {...register("province", { required: "La provincia es necesaria para ubicar el negocio" })} onChange={(e) => onChangeProvince(e)}>
            ...
            <div className="flex flex-col md:flex-row md:gap-2">
                ...
                <div>
                    ...
                    <select className="w-full p-2 mt-1 rounded-lg border" {...register("distrito", { required: "El distrito es necesario para ubicar el negocio" })} onChange={(e) => onChangeDistrito(e)} >
                ...
                <div>
                    ...
                    <select className="w-full p-2 mt-1 rounded-lg border"  {...register("corregimiento", { required: "El corregimiento es necesario para ubicar el negocio" })}>
                        <option value="">Seleccione Distrito</option>
